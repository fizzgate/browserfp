...
def make_firefox_profile(cert_path=None):
    ...
    if cert_path is None:
        cert_path = CA_CERT
    profile = tempfile.mkdtemp(prefix="browserfp-ffprof-")
    subprocess.run([cu, "-N", "-d", f"sql:{profile}", "--empty-password"],
                   check=True, capture_output=True)
    subprocess.run([cu, "-A", "-n", "browserfp-observer", "-t", "C,,",
...
def collect_real(with_safari=False):
    ...
    for name, engine, binary, version in discover():
        ...
        if engine != "chromium":
            ...
            continue
        profile = tempfile.mkdtemp(prefix=f"browserfp-h2-{name}-")
        with H2Probe() as probe:
            p = subprocess.Popen(
...
SAFARI_CA_LABEL = "browserfp-observer-CA"
