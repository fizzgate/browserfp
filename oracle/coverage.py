...
def main():
    if not os.path.exists(GOLDEN) or not os.path.exists(REAL):
        print("缺 golden：先跑 `python -m oracle.collect --no-sni` 和 "
              "`python -m oracle.browsers`", file=sys.stderr)
        return 2
    ...
    print(f"curl_cffi target: {len(golden)}    真机样本: {len(real)}\n")
