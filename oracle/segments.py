...
def segment_substitutable(seg, golden):
    ...
    **别用"全品牌扩展出现率"去找数据不全的记录**——试过，会把老版本误判掉。
    实测 tls_client 的 firefox_110 与 firefox_120 缺 0x23(session_ticket) 与
    0x2d(psk_key_exchange_modes)，而同品牌其余记录都有，看着像采集不全；但按
    出现率筛的话，2020 年的 firefox 78 也会被算成"缺了常见扩展"——它只是那时
    还没有 0x1b、0xfe0d 而已。比较基准得是同年代的版本，不是全品牌。
