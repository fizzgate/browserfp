...
| 指标 | 数值 |
|---|---|
| 唯一指纹 | **79**（来自 314 个 target 名，按 13 个确定性字段去重） |
...
扫描结果（这些版本生产流量里没出现过），合计缺 33 个：

| 品牌 | 已发布版本数 | 缺 | 成因 |
|---|---|---|---|
| chrome | 83 | 71, 97–98, 153 | 71/153 段内无 golden；97–98 段内数据矛盾 |
| chrome-mobile | 83 | 71, 153 | 同上 |
| firefox | 76 | 83–84, 111, 119, 121–122 | 83–84 无 golden；其余段内数据矛盾 |
| firefox-mobile | 76 | 83–84, 111, 119, 121–122, 145–153 | 同上 + 145 后与桌面不同形态且无 golden |
...
2. **段内实采数据互相矛盾** —— 不是判段维度不足。实测 chrome 段 97–118 里
   ...
   这样，只能是抓包时落在不同 Finch 实验组。这类段保持不可替代：**段内有互相
   矛盾的实采数据时，"不伪装"比"挑一个用"正确**。
