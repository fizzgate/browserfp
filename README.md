...
#### 还剩一处同族的：GREASE ECH 的长度

A/B 修完 padding 再跑，`chrome119` 仍然不符，**但方向翻了**（原来我们多一个扩展，
现在本尊多一个）。本地 sniffer 抓本尊的真实字节逐扩展比长度，答案很干脆：

```
无 SNI          CH体=512  SNI体=0   ECH体=218
SNI=localhost   CH体=588  SNI体=14  ECH体=282
```

**ECH 的 payload 长度也随报文长度变**（差 64 = 2×32，而 SNI 只差 18），是按内层
ClientHello 补齐到 32 的倍数算的 —— 我们却沿用了 golden 的长度。而 ECH 长度进
总长、总长决定 padding 补不补，所以它把 padding 的判定也带偏了。

这是同一族的**第 6 处**，单独记成待办（见 `TaskCreate` 的记录），判据现成：
A/B 臂 + 本地 sniffer 逐扩展比长度。**不把它藏起来**：`test_echo_fingerprint`
属于联网层，默认 `verify_all` 不跑它，但只要跑 `--live` 它就会报出来。
