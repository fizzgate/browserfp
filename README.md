...
| 指标 | 数值 |
|---|---|
| 唯一指纹 | **82**（来自 320 个 target 名，按 13 个确定性字段去重） |
...
缺 webkit（Safari）—— 无强制开关，且自签 CA 需改系统信任
```

配了引擎级棘轮（`QUIC_ENGINES`）：少一个引擎就红。变异验过 —— 删掉
`real_quic:firefox` 会红，只抹掉它的 `h3` 字段也会红（两层分别记）。

Safari 那条**不做**：它既没有强制走 h3 的开关，自签 CA 又要改用户/系统的信任
设置才认 —— 那是侵入性操作。TCP 那一端对它已经是现成的，缺的只是证书信任。
