/* browserfp —— ClientHello 解析与 JA4 计算（C 实现）
 *
...
/* HTTP/2 连接开场。**独立于 browserfp_profile 按 (品牌,版本) 查**，不挂在 TLS
...
typedef struct {
    ...
    /* 该 profile 属于哪个引擎。实测 81 条无一跨引擎，所以良定义 ——
       它是两层一致性检查（browserfp_coherence）的基础，**也决定要不要每连接打乱
...
typedef struct {
    ...
    uint16_t    profile;    /* 下标，指向 browserfp_profiles[] */
 ...
 * 版本口径与 browserfp_lookup_ua 一致：Chromium 系衍生浏览器传内核 Chrome 版本。 */
 ...
 * 曾经有过 order_csv 参数与配套的 browserfp_engine_of_headers / header_order /
...
typedef struct {
    ...
    browserfp_u16list ciphers;          /* 已剔除 GREASE */
    browserfp_u16list extensions;       /* 已剔除 GREASE，保持线上顺序 */
...
/* 返回 0 成功，负数为错误码 */
int browserfp_parse_client_hello(const uint8_t *record, size_t len,
                               browserfp_hello *out);
...
static inline int browserfp_is_grease(uint16_t v) {
