...
实测第一次跑就抓到 `browserfp_ja4(NULL, …)` 直接 SEGV（少一个空指针检查）。

喂进去的东西：NULL、空串、单空格、超长串（70KB）、非 UTF-8 字节、全逗号、
越界与 (size_t)-1 下标、零长缓冲、差一字节的缓冲、截断到每一个长度的 TLS
record。

**外加结构化变异**：拿库里每条 profile 真实构造出来的 ClientHello **逐位点**
改，每个位点试 6 种恶意值，再各配"长度说得比实际多/少"两种截断，以及一次
与相邻字节的**双字节**变异 —— 长度字段是 2 字节的，只改一个字节常常还落在
合法范围里，而真实攻击也不会只动一处。全零与截断只能
