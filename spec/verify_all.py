...
SLOW_GATES = {
    ...
    "test_h2_table": 900,
    # **首次跑要现编 Go 服务端**（hrrserver / h2echo 等，二进制不入库）。
    # 新克隆的第一次会把 go build 的时间算进来，300s 默认预算不够 —— 实测
    # 一次 verify_all 里报「1 处问题」，而单独跑与重跑都是绿的：那次正是
    # hrrserver 还没编出来。给足预算，别让「第一次跑」看起来像代码有问题。
    "test_hrr": 900,
    "test_match": 900,
}
